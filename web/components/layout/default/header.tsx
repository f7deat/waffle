...
const DefaultHeader: React.FC = () => {
    ...
    const { header, footer, menu } = useAppContext();

    const brand = header?.brand || "DefZone.Net";
    const logo = header?.logo;
    const searchPlaceHolder = header?.searchPlaceHolder || "Tìm kiếm...";
    const topMenu = header?.topMenu || [];
    const social = footer?.social;

    const navItems = menu.filter((item) => item.active !== false);
            ...
            <div className="hidden border-b border-slate-200 bg-slate-50 text-xs text-slate-600 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300 md:block">
                <div className="mx-auto flex container items-center justify-between px-4 py-2">
                    <div className="flex items-center gap-4">
                        {topMenu.length > 0 ? (
                            topMenu.map((item) => (
                                <a key={item.href} href={item.href} target={item.target} className="hover:text-slate-900 dark:hover:text-white">{item.name}</a>
                            ))
                        ) : (
                            <>
                    ...
                    <div className="flex items-center gap-3 text-slate-500 dark:text-slate-300">
                        {social?.facebookUrl && (
                            <a href={social.facebookUrl} aria-label="Facebook" className="hover:text-blue-600">
                                <FacebookFilled />
                            </a>
                        )}
                        ...
                        {social?.instagramUrl && (
                            <a href={social.instagramUrl} aria-label="Instagram" className="hover:text-red-500">
                                <InstagramFilled />
                            </a>
                        )}
                    ...
                    <Link href="/" className="flex items-center gap-3">
                        {logo ? (
                            ...
                            <div>
                                ...
                                <div className="text-lg font-semibold text-slate-900 dark:text-slate-100">{brand}</div>
                    ...
                    <div className="absolute inset-y-0 left-0 w-72 max-w-[80vw] bg-white shadow-xl dark:bg-slate-950">
                        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-slate-800">
                            <div className="flex items-center gap-2">
                                {logo ? (
                                    // eslint-disable-next-line @next/next/no-img-element
                                    <img src={logo} alt={brand} className="h-10 w-10 rounded-full object-cover" />
                                ) : (
                                    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-blue-600 text-white font-semibold">
                                        {brand.slice(0, 2).toUpperCase()}
                                    </div>
                        ...
                        <div className="px-4 py-4 space-y-2">
                            ...
                            <div className="flex items-center gap-3 pt-2 text-slate-600 dark:text-slate-200">
                                {footer?.email && <a href={`mailto:${footer.email}`} className="text-sm">{footer.email}</a>}
                                <div className="flex items-center gap-3">
                                    ...
                                    {social?.xUrl && (
                                        <a href={social.xUrl} aria-label="X" className="hover:text-sky-500">
                                            <svg aria-hidden="true" viewBox="0 0 24 24" className="h-5 w-5" fill="currentColor">
                                    ...
                                    {social?.youtubeUrl && (
                                        <a href={social.youtubeUrl} aria-label="YouTube" className="hover:text-red-500">
                                            <svg aria-hidden="true" viewBox="0 0 24 24" className="h-5 w-5" fill="currentColor">
