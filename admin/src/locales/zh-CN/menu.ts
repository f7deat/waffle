export default {
  'menu.catalog.tag': 'Tag',
  'menu.catalog.tagCenter': 'Tag center',
  'menu.catalog': '目录',
  'menu.comments': '评论',
  'menu.partner': '合作伙伴',
  'menu.hr': '人力资源',
  'menu.hr.statistics': '统计',
  'menu.hr.branch': '分支',
  'menu.hr.department': '部门',
  'menu.hr.team': '团队',
  'menu.hr.teamMembers': '团队成员',
  'menu.ecommerce': 'E-Commerce',
  'menu.ecommerce.order': 'Orders',
  'menu.ecommerce.newOrder': 'New order',
  'menu.component': 'Components',
  'menu.component.articleLister': 'Article Lister',
  'menu.component.articlePicker': 'Article Picker',
  'menu.component.articleSpotlight': 'Article Spotlight',
  'menu.component.affiliateLink': 'Affiliate Link',
  'menu.component.block': '堵塞',
  'menu.component.blockEditor': 'Block editor',
  'menu.component.blogger': 'Blogger',
  'menu.component.card': 'Card',
  'menu.component.contactForm': 'Contact form',
  'menu.component.facebookAlbum': 'Facebook album',
  'menu.component.image': 'Image',
  'menu.component.googleMap': 'Google Map',
  'menu.component.link': 'Link',
  'menu.component.listGroup': 'List Group',
  'menu.component.lookbook': 'Lookbook',
  'menu.component.masonry': 'Masonry',
  'menu.component.navbar': 'Navbar',
  'menu.component.productLister': 'Product Lister',
  'menu.component.productPicker': 'Product Picker',
  'menu.component.productSpotlight': 'Product Spotlight',
  'menu.component.shopeeProduct': 'Shopee Product',
  'menu.component.swiper': 'Swiper',
  'menu.component.row': 'Row',
  'menu.component.tag': 'Tag',
  'menu.component.trend': 'Trend',
  'menu.component.exchangeRate': 'Exchange Rate',
  'menu.component.videoPlayer': 'Video Player',
  'menu.component.wordPressLister': 'WordPress Lister',
  'menu.fileManager': '文件管理器',
  'menu.fileManager.fileCenter': 'File center',
  'menu.dashboard': '仪表板',
  'menu.help': '帮助',
  'menu.help.backup': '备份',
  'menu.help.upgrade': '升级',
  'menu.help.logs': '日志',
  'menu.tool': '工具',
  'menu.tool.shortLink': '短链接',
  'menu.tool.blocksuite': 'BlockSuite',
  'menu.tool.websiteBuilder': '网站构建器',
  'menu.login': '登录',
  'menu.shop': 'E-Commerce',
  'menu.shop.productCenter': 'Product center',
  'menu.settings': '环境',
  'menu.settings.component': '成分',
  'menu.settings.style': '主题',
  'menu.settings.general': '常规设置',
  'menu.settings.google': 'Google',
  'menu.settings.header': 'Header',
  'menu.settings.telegram': 'Telegram',
  'menu.settings.facebook': 'Facebook',
  'menu.settings.componentCenter': 'Component center',
  'menu.settings.localization': '语言',
  'menu.settings.sendGrid': 'SendGrid',
  'menu.setting.category': 'Category',
  'menu.user': '用户',
  'menu.user.contact': '接触',
  'menu.user.member': '成员',
  'menu.user.profile': '轮廓',
  'menu.user.role': '角色',
  'menu.user.roleCenter': 'Role Center',
  'menu.user.userCenter': 'User center',
};
