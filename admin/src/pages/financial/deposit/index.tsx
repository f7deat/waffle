import { apiFinancialDeposit, apiFinancialDeposits, apiFinancialUserOptions } from '@/services/financial';
import { PageContainer, ProCard } from '@ant-design/pro-components';
import { Button, Col, Form, Input, InputNumber, message, Row, Select, Table } from 'antd';
...
const DepositPage: React.FC = () => {
    ...
    const loadHistory = async (current = pagination.current, pageSize = pagination.pageSize) => {
        setLoading(true);
        try {
            const response = await apiFinancialDeposits({ current, pageSize });
    ...
    const onFinish = async (values: { userId: string; amount: number; note?: string }) => {
        setSubmitting(true);
        try {
            const response = await apiFinancialDeposit({
    ...
    const columns: ColumnsType<API.FinancialTransaction> = useMemo(
        () => [
            ...
            },
            {
                ...
                align: 'right',
                width: 170,
                render: (value: number) => <span className='text-green-600 font-semibold'>{formatMoney(value)}</span>,
            },
            {
    ...
    return (
        <PageContainer title='Financial Deposit'>
            <ProCard title='Tạo giao dịch nạp tiền' className='mb-4'>
                    ...
                    <Row gutter={16}>
                        ...
                        <Col xs={24} md={8}>
                            <Form.Item label='Ghi chú' name='note'>
                                <Input allowClear placeholder='Ví dụ: nạp thưởng chiến dịch tháng 7' />
                            </Form.Item>
                        </Col>
                        <Col xs={24}>
            ...
            <ProCard title='Lịch sử nạp tiền'>
